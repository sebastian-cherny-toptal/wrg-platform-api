...
export async function clearPreviousBatonRougeSeed(
  ...
  log("Cleanup starting with independent, idempotent deletes.");

  // Order.organizationId is required and uses RESTRICT, so seeded test orders
  // must be removed before their seeded organizations can be replaced. Keep
  // these as separate statements: the project cascade can take several minutes
  // in production and must not be bounded by an interactive transaction timeout.
  ...
  log(`Cleanup: deleted ${deletedOrganizations.count} organization(s).`);
  log(`Cleanup completed in ${Date.now() - startedAt}ms.`);
}
