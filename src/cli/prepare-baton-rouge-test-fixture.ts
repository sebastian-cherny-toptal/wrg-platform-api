...
function parseOptions(argv: string[]): Options {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (!argument || !["--source", "--report-source", "--output"].includes(argument) || !value) {
...
function sanitizedValue(value: ExcelJS.CellValue, header: string, year: number): ExcelJS.CellValue {
  ...
  if (typeof value === "number") return sensitiveColumn ? `Synthetic ${digest(value)}` : value;
  if (typeof value === "boolean") return value;

  const text = typeof value === "string" ? value.trim() : String(value);
  if (!text) return null;
  if (!sensitiveColumn && /^-?\d+(?:\.\d+)?$/u.test(text)) return Number(text);
  if (/^(?:yes|no|n\/a|not applicable|true|false|complete|completed|incomplete|en|es|fr)$/iu.test(text)) {
...
async function sanitizedRawWorkbook(bytes: Buffer, year: number): Promise<Buffer> {
  ...
  for (const worksheet of workbook.worksheets) {
    ...
    worksheet.eachRow((row, rowNumber) => {
      ...
      row.eachCell({ includeEmpty: false }, (cell, column) => {
        cell.value = sanitizedValue(cell.value, headers.getCell(column).text.trim(), year);
      });
    });
  }
...
async function main(): Promise<void> {
  ...
  if (years.size === 0) throw new Error("No Baton Rouge source workbooks were found");

  const reportFiles = readdirSync(options.reportSource).filter((fileName) => {
    const containsYear = [...years].some((year) => fileName.includes(String(year)));
    return (
      containsYear &&
      /(?:workforce\s+benchmark|benchmark\s+comparisons|benefits\s*&\s*best\s*practices).*\.xlsx$/iu.test(fileName)
    );
  });
  for (const fileName of reportFiles) {
    output.addFile(fileName, await metadataCleanWorkbook(join(options.reportSource, fileName)));
  }
  if (reportFiles.length !== years.size * 2) {
    throw new Error(`Expected ${years.size * 2} published reports, found ${reportFiles.length}`);
  }
  ...
  console.log(`Only synthetic row values and aggregate published reports are included; source data remains outside Git.`);
}
