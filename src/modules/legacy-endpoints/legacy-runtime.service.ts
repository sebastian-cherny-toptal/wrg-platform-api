...
const legacyRequire = createRequire(
  path.join(nativeLegacyRoot, "package.json"),
);
...
class NativeResponse {
  ...
  get sent(): boolean {
    return this.completed || Boolean(this.reply.sent);
  }
  ...
  download(
    ...
    const filename =
      typeof filenameOrCallback === "string"
        ...
        : path.basename(file);
    const done =
      typeof filenameOrCallback === "function" ? filenameOrCallback : callback;
    this.reply.header(
      "Content-Disposition",
    ...
    if (!this.initPromise) {
      this.initPromise = (async () => {
        ...
        try {
          ...
          this.logger.warn(
            `Legacy MongoDB initialization failed; requests will fail until it is available: ${String(error)}`,
          );
        }
      })();
    }
  ...
  async handle(request: AnyRecord, reply: AnyRecord): Promise<void> {
    ...
    if (!route)
      throw new NotFoundException(
        `Legacy endpoint not found: ${req.method} ${url}`,
      );
    if (route.controller !== "inline") await this.initialize();
    const cleanup = await this.parseMultipart(req);
    try {
      for (const middleware of route.middleware ?? []) {
        const continued = await this.runMiddleware(
          this.middlewareFor(middleware),
          req,
          res,
        );
        if (!continued) return;
      ...
      const target =
        route.controller === "inline"
          ? {
              ok: (_req: AnyRecord, response: NativeResponse) =>
                response.send("cool"),
            }
